  ...
  export default {
    ...
    methods: {
      loadPosts(defaultPageSize) {
        const postKeyword = this.$store.getters.postKeyword
        const postCategoryId = this.$store.getters.postCategoryId
        const postListStatus = this.$store.getters.postListStatus

        this.postsLoading = true
        // Set from pagination
        this.queryParam.page = this.pagination.page - 1

        if(defaultPageSize&&!postKeyword&&!postCategoryId&&!postListStatus){
          this.queryParam.size = defaultPageSize
        }else{
        ...
        this.queryParam.keyword = postKeyword
        this.queryParam.categoryId = postCategoryId
        this.queryParam.status = postListStatus
      ...
      handleResetParam() {
        this.$store.dispatch('setPostKeyword', null)
        this.$store.dispatch('setPostCategoryId', null)
        this.$store.dispatch('setPostStatus', null)
      ...
      handleQuery() {
        // 将列表参数储存到store里，方便从其他页面跳到当前页面能自动通过之前的查询参数过滤列表，而不是重新点选
        this.$store.dispatch('setPostKeyword', this.queryParam.keyword)
        this.$store.dispatch('setPostCategoryId', this.queryParam.categoryId)
        this.$store.dispatch('setPostStatus', this.queryParam.status)
